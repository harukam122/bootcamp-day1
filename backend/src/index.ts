...
app.post("/posts/:postID/comments", async (req, res) => {
    ...
    const postBodyData = req.body;
    const collection = db.collection("comments");
    const newComment = {content: postBodyData.content, post: new ObjectId(postID), postedAt: new Date()};
    try {
...
function start() {
    const client = new MongoClient("mongodb+srv://harukam_122:140122Mashar0122!@test.nswbu.mongodb.net/database?retryWrites=true&w=majority");
    client.connect()
        .then(() => {
